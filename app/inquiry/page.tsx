...
export default function InquiryPage() {
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <section className="bg-gradient-to-b from-gray-50 to-white border-b border-gray-200 py-12 sm:py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-12 text-center">How to order</h2>
            ...
            <div className="flex items-stretch gap-2 lg:gap-4">
              {HOW_TO_STEPS.map((step, i) => (
                <div key={i} className="flex flex-1 items-center min-w-0">
                  {/* Step card */}
                  <div className="inquiry-speakable flex-1 flex flex-col items-center p-5 bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:border-gray-400 transition-all text-center min-h-[130px]">
                    <span className="w-10 h-10 rounded-full bg-gray-900 text-white flex items-center justify-center text-sm font-bold shrink-0">{i + 1}</span>
                    <h3 className="mt-3 font-semibold text-gray-900 text-sm">{step.title}</h3>
                    <p className="mt-1.5 text-xs text-gray-600 leading-relaxed">{step.text}</p>
                  </div>
                  {/* Arrow between steps */}
